...
public/
  ...
  captions.js       # seed + offline fallback; the Worker generates the live one from KV
...
## Photo manifest (`/captions.js`)

The live `/captions.js` is **generated by the Worker from the KV key `photos`**, so photos and
captions can change without a deploy. `public/captions.js` is still committed, and is the seed
and the offline fallback: when KV has no `photos` key the Worker serves the static asset instead.

`wrangler.jsonc` lists `assets.run_worker_first: ["/captions.js"]`. This is required — static
assets normally win before the Worker runs, so without it the Worker could never intercept a
path that also exists in `public/`.

> **Never add `/images/*` to `run_worker_first`.** Static asset requests are free and unlimited;
> Worker invocations are capped at 100k/day on the free plan. One gallery page view fans out to
> dozens of images, so routing them through the Worker would burn the budget for no benefit.
> Verified locally: 10 image requests produce 0 Worker invocations, 1 `/captions.js` produces 1.

The manifest is `[{ n, caption, src, ts }]` and is **append-only — photos are never deleted**.
That is what keeps plate numbers permanently stable, which matters because comments
(`comments:<plate>`) and pet counts (the `plates` map) are keyed by plate number. `src` is
`images/dog-N.jpeg` for photos committed to the repo and `/photos/dog-N.jpeg` for uploads.

The generated file sets three globals:

```js
window.WRANGELL_CAPTIONS  // index 0 = plate 1 — unchanged, for back-compat
window.WRANGELL_PHOTOS    // authoritative manifest; read `src` from here, not by string-building
window.WRANGELL_BULLETIN  // the currently-active bulletin, or null
```
